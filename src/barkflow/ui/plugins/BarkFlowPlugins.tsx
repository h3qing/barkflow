...
interface BarkFlowElectronAPI {
  barkflowGetPlugins: () => Promise<Plugin[]>;
  barkflowUpdatePlugin: (id: string, updates: Partial<Plugin>) => Promise<Plugin | null>;
  barkflowAddPlugin: (config: Plugin) => Promise<Plugin | null>;
  barkflowRemovePlugin: (id: string) => Promise<{ success: boolean }>;
}
...
interface BarkFlowPluginsProps {
...
function AddPluginForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-primary/20 bg-primary/5 dark:bg-primary/8 p-4 space-y-3">
      ...
      <input value={command} onChange={(e) => setCommand(e.target.value)} placeholder="Command (e.g. npx @barkflow/my-plugin)" className={inputClass} />
...
export default function BarkFlowPlugins({ className }: BarkFlowPluginsProps) {
  ...
  const fetchPlugins = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await getAPI().barkflowGetPlugins();
      setPlugins(data ?? []);
    } catch {
  ...
  const handleToggle = useCallback(async (id: string, currentEnabled: boolean) => {
    try {
      const updated = await getAPI().barkflowUpdatePlugin(id, { enabled: !currentEnabled });
      if (updated) {
  ...
  const handleAdd = useCallback(async (input: { name: string; command: string; description: string }) => {
    try {
      ...
      const result = await getAPI().barkflowAddPlugin(config);
      if (result) {
  ...
  const handleRemove = useCallback(async (id: string) => {
    try {
      await getAPI().barkflowRemovePlugin(id);
